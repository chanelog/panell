  ...
  "inbounds": [
    {"port": 443, "protocol": "vless",
     ...
     "streamSettings": {"network": "ws", "security": "tls",
       ...
       "tlsSettings": {"certificates": [{"certificateFile": "/etc/xray/ssl/fullchain.pem", "keyFile": "/etc/xray/ssl/privkey.pem"}]}}},
    {"port": 80, "protocol": "vless",
     "settings": {"clients": [], "decryption": "none"},
     "streamSettings": {"network": "ws", "security": "none", "wsSettings": {"path": "/vless"}}},
    {"port": 2083, "protocol": "vmess",
     "settings": {"clients": []},
     "streamSettings": {"network": "ws", "security": "tls",
       "wsSettings": {"path": "/vmess"},
       "tlsSettings": {"certificates": [{"certificateFile": "/etc/xray/ssl/fullchain.pem", "keyFile": "/etc/xray/ssl/privkey.pem"}]}}},
    {"port": 8880, "protocol": "vmess",
     "settings": {"clients": []},
     "streamSettings": {"network": "ws", "security": "none", "wsSettings": {"path": "/vmess"}}},
    {"port": 2096, "protocol": "trojan",
     "settings": {"clients": []},
     "streamSettings": {"network": "ws", "security": "tls",
...
menu_ssh() {
    ...
    case "$opt" in
        1)
            ...
            SQUID_PORT=$(ss -tlnp 2>/dev/null | grep -E '3128|8080' | awk '{print $4}' | rev | cut -d: -f1 | rev | head -1); SQUID_PORT=${SQUID_PORT:-3128}
            HAPROXY_PORT=$(ss -tlnp 2>/dev/null | grep haproxy | awk '{print $4}' | rev | cut -d: -f1 | rev | sort -n | tr '\n' ',' | sed 's/,$//')
            [ -z "$HAPROXY_PORT" ] && HAPROXY_PORT="443, 80"
            NOOBZ_PORT=$(ss -tlnp 2>/dev/null | grep noobz | awk '{print $4}' | rev | cut -d: -f1 | rev | head -1); NOOBZ_PORT=${NOOBZ_PORT:-40000}
            UDP_PORT="36712"
            echo -e "\n${GREEN}╔═══════════════════════════════════════════╗${NC}"
            ...
            echo -e "${GREEN}╠═══════════════════════════════════════════╣${NC}"
            echo -e "${GREEN}║${NC}${YELLOW}         PORT SEMUA PROTOKOL SSH           ${NC}${GREEN}║${NC}"
            echo -e "${GREEN}╠═══════════════════════════════════════════╣${NC}"
            echo -e "${GREEN}║${NC} SSH OpenSSH     : ${WHITE}${SSH_PORT}${NC}"
            echo -e "${GREEN}║${NC} Dropbear SSH    : ${WHITE}${DROPBEAR_PORT}${NC}"
            echo -e "${GREEN}║${NC} NoobzVPN        : ${WHITE}${NOOBZ_PORT}${NC}"
            echo -e "${GREEN}║${NC} UDP Custom      : ${WHITE}${UDP_PORT}${NC}"
            echo -e "${GREEN}║${NC} HAProxy (SSL)   : ${WHITE}${HAPROXY_PORT}${NC}"
            echo -e "${GREEN}║${NC} Proxy/HTTP      : ${WHITE}${SQUID_PORT}${NC}"
            echo -e "${GREEN}╚═══════════════════════════════════════════╝${NC}"
            ;;
        2)
...
menu_vmess() {
    ...
    case "$opt" in
        1)
            ...
            # nTLS / non-TLS (port 80)
            JSON_NTLS="{\"v\":\"2\",\"ps\":\"${u}-ntls\",\"add\":\"${DOMAIN}\",\"port\":\"80\",\"id\":\"${UUID}\",\"aid\":\"0\",\"net\":\"ws\",\"type\":\"none\",\"host\":\"${DOMAIN}\",\"path\":\"/vmess\",\"tls\":\"\"}"
            ...
            echo -e "${GREEN}╠═══════════════════════════════════════════════════════╣${NC}"
            echo -e "${GREEN}║${NC} ${YELLOW}[TLS]${NC}  Port: ${WHITE}443${NC}  Path: ${WHITE}/vmess${NC}  Net: ${WHITE}ws${NC}"
            echo -e "${GREEN}║${NC} ${YELLOW}[nTLS]${NC} Port: ${WHITE}80${NC}   Path: ${WHITE}/vmess${NC}  Net: ${WHITE}ws${NC}"
...
menu_vless() {
    ...
    case "$opt" in
        1)
            ...
            # nTLS WS (port 80)
            LINK_NTLS="vless://${UUID}@${DOMAIN}:80?encryption=none&security=none&type=ws&host=${DOMAIN}&path=%2Fvless#${u}-ntls"
            # gRPC TLS
            ...
            echo -e "${GREEN}║${NC} Domain   : ${WHITE}$DOMAIN${NC}"
            echo -e "${GREEN}╠═══════════════════════════════════════════════════════╣${NC}"
            echo -e "${GREEN}║${NC} ${YELLOW}[TLS]${NC}  Port: ${WHITE}443${NC}  Path: ${WHITE}/vless${NC}   Net: ${WHITE}ws${NC}"
            echo -e "${GREEN}║${NC} ${YELLOW}[nTLS]${NC} Port: ${WHITE}80${NC}   Path: ${WHITE}/vless${NC}   Net: ${WHITE}ws${NC}"
...
menu_trojan() {
    ...
    case "$opt" in
        1)
            ...
            # nTLS WS (port 80)
            LINK_NTLS="trojan://${p}@${DOMAIN}:80?security=none&type=ws&host=${DOMAIN}&path=%2Ftrojan#${u}-ntls"
            # gRPC TLS
